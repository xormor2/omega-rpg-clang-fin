...
void sign_print(int x, int y, int signp)
{
  ...
  case L_CHARITY:
    print1("Havaitset kyltin: Rampartin orpokoti ja köyhien asuntola.");
    break;
  case L_MANSION:
...
{
  long donation;
  print2("'Tervehdys, ystävä. Haluatko tehdä lajoituksen?' [yn] ");
  if (ynq2()!='y') 
    print3("'Pinchpurse!'");
  else {
    clearmsg();
...
void l_tourist(void)
{
   print1("Greetings and welcome to Rampart. Tervehdys ja tervetuloa Rampartiin.");
   print2("Would you like the tourist guide to the city? Haluatko turistioppaan? [yn] [ke]");
   char answer_question=ynq();
   if( (answer_question == 'y') || (answer_question == 'k') )
   {
     menuclear();
     ...
     clearmsg();
     xredraw();
     print1("Have a nice day. Mukavaa päivänjatkoa.");
   } else
   {
     clearmsg();
     print1("Have a nice day then. Hyvää päivänjatkoa sitten.");
   }
}
