...
void tunnelcheck(void)
{
  ...
  if ((Level->tunnelled) > (Level->level_length)/4)
    mprint("Dust and stone fragments fall on you from overhead. Pölyä ja soraa tippuu päällesi.");
  if ((Level->tunnelled) >(Level->level_length)/2)
    mprint("You hear groaning and creaking noises. Kuulet narinaa.");
  if ((Level->tunnelled) > (3*(Level->level_length))/4) 
    mprint("The floor trembles and you hear a loud grinding screech. Lattia tärisee ja kuulet kiven pettävän.");
  if ((Level->tunnelled) > Level->level_length) {
    mprint("With a scream of tortured stone, the entire dungeon caves in!!! Koko luola romahtaa!!!");
	...
	case PALACE:
      ...
      print1("In your shadowy state, you float back up to the surface. Varjomaisessa tilassasi leijut takaisin pinnalle.");
      return;
    }
    mprint("You are flattened into an unpleasant jellylike substance. Mösähdät hyytelöksi.");
...
void showroom(int i)
{
  ...
  switch(Current_Environment) {
    ...
    case E_HOVEL:
      ...
      break;
    case E_CITY:
      strcpy(Str2,"The City of Rampart, Linnavuoren kaupunki");
      break;
    case E_VILLAGE:
      switch(Villagenum) {
...
int p_moveable(int x, int y)
{
  ...
  if (! inbounds(x,y)) return (FALSE);
	...
	if (Level->site[x][y].locchar != WATER ||
	  ...
	  print1("You can't convince your steed to continue. Ratsusi ei jatka matkaa.");
	  setgamestatus(SKIP_MONSTERS);
	  return(FALSE);
	}
	else return(TRUE);
...
void searchat(int x, int y)
{
  ...
  if (inbounds(x,y) && (random_range(3) || Player.status[ALERT])) {
    if (loc_statusp(x,y,SECRET)) {
	  ...
	  (Level->site[x][y].locchar==CLOSED_DOOR)) {
	mprint("You find a secret door! Löydät salaoven!");
	for(i=0;i<=8;i++) { /* FIXED! 12/25/98 */
	  lset(x+Dirs[0][i],y+Dirs[1][i],STOPS);
	  lset(x+Dirs[0][i], y+Dirs[1][i], CHANGED);
	}
      }
      else mprint("You find a secret passage! Löydät salaisen polun!");
...
void fight_monster(struct monster *m)
{
  ...
  if (Player.status[AFRAID]) {
    print3("You are much too afraid to fight! Et uskalla taistella!");
    reallyfight = FALSE;
  }
  else if (player_on_sanctuary()) {
    print3("You restrain yourself from desecrating this holy place. Et halua häpäistä tätä pyhää paikkaa.");
    reallyfight = FALSE;
  }
  else if (Player.status[SHADOWFORM]) {
    print3("Your attack has no effect in your shadowy state. Varjomaisessa tilassasi hyökkäykselläsi ei ole vaikutusta.");
    reallyfight = FALSE;
  }
