...
static int bank_new_customer (WINDOW *w)
{
  ...
  mvwaddstr(w, 4, 2, "Onko sinulla tunnukset? [yn] : ");
  wrefresh(w);
...
static void bank_call_police (void)
{
  ...
  cinema_scene("A loud siren goes off! Sireeni ulvoo!", 0, 0);
...
static pob bank_open_account (WINDOW *w)
{
  ...
  mvwaddstr(w, 5, 2, "Haluatko avata tilin? [yn] : ");
  wrefresh(w);
  ...
  while (1)
    {
      ...
      if ('Y' == response)
        ...
        {
          waddstr(w, "no");
          mvwaddstr(w, 7, 2, "        Muista meidät tulevassa       ");
          mvwaddstr(w, 8, 2, "     kun tarvitset pankkipalveluita    ");
          wrefresh(w);
          sleep(4);
          return 0;
        }
...
void l_bank (void)
{
  ...
  mvwaddstr(Bankw, 2, 2, "         Tervetuloa Rampartin pankkiin        ");

  /* bank working? or not? */
  if (gamestatusp(BANK_BROKEN))
    {
      mvwaddstr(Bankw, 9, 2, "            Pankkiautomaatti pois käytöstä            ");
      wrefresh(Bankw);
      cinema_scene("You see a damaged autoteller. Näet vahingoittuneen pankkiautomaatin.", 0, 0);
    }
  else if (police_are_hostile())
    {
      mvwaddstr(Bankw, 9, 2, "      Pankkiautomaatti pois käytöstä       ");
      wrefresh(Bankw);
      cinema_scene("Ilmeisesti pankki on kiinni.", 0, 0);
    }
  else
    {
      int is_new_customer;
